...
export function normalizeGuestsState(value: OffreWidgetGuestsState): OffreWidgetGuestsState {
...
export function isPersistedGuestsState(value: unknown): value is OffreWidgetGuestsState {
...
export function resolveGuestsStorageKey(key: string | null | undefined) {
...
export function readPersistedGuestsState(key: string | null | undefined) {
...
export function writePersistedGuestsState(key: string | null | undefined, value: OffreWidgetGuestsState) {
  const storageKey = resolveGuestsStorageKey(key);
...
export function buildSelectedRoomCriterias(selectedGuests: OffreWidgetGuestsState): WidgetRoomCriteria[] {
  return [{
    passengers: [
      ...Array.from({ length: selectedGuests.adultsCount }, () => ({
        age: 20,
        passengerType: 0
      })),
      ...selectedGuests.childrenAges.map((age) => ({
        age,
        passengerType: 1
      }))
    ]
